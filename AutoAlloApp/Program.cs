...
namespace AutoAlloApp
{
    ...
    static class Program
    {
        ...
        public static AllocateAlgorithm original = AllocateAlgorithm.CustomOrderFill_A_to_G_First;
        public static AllocateAlgorithm allocationAlorithm;
        ...
        static float originalSinglePercent = 0.7f;
        static float singlePercent;

        public static float originalScale = 1.5f;
        ...
        static void Main(string[] args)
        {
            ...
            while ( i < 29 ) {
                ...
                allocationAlorithm = original;
                singlePercent = originalSinglePercent;
                scale = originalScale;
                
                EXPORTLOCATION = (AppDomain.CurrentDomain.BaseDirectory + $"{i}.csv").Replace("AutoAlloApp\\bin\\Debug\\net5.0\\", "");
                i++;
                ...
                while (buildings.Any(x => x.PercentageAllocated < 1))
                {
                    ...
                    switch (allocationAlorithm)
                    {
                        ...
                        case AllocateAlgorithm.CustomOrderFill_A_to_G_First:
                            ...
                            singlePercent = (float)spots.Where(x => !x.Key.Contains("U2")).Count() / (float)spots.Count();
                ...
                //Optional. Just for testing multiple days
                CreateOldExportFile();

            }

            File.Delete(OLDEXPORTLOCATION);
        }
        ...
        private static void PrintData()
        {
            ...
            Console.WriteLine($"{avereageDistance / buildings.Count}                          {highest - lowest}");
        }
        ...
        /// <summary>
        /// Creates the csv that would be used the next day. /!\ THIS IS JUST FOR TESTING PURPOSES /!\
        /// </summary>
        private static void CreateOldExportFile()
        {
            string[] lines = new string[reservations.Count];

            for (int i = 0; i < reservations.Count; i++)
            {
                Reservation res = reservations[i];
                lines[i] = $"{res.ParkingSpot};Person;91911217;a@b.com;{res.PersonKey};{res.RoomKey};{res.ArrivalDate.Split("-")[2]}.{res.ArrivalDate.Split("-")[1]}.{res.ArrivalDate.Split("-")[0]};{res.DepartureDate.Split("-")[2]}.{res.DepartureDate.Split("-")[1]}.{res.DepartureDate.Split("-")[0]};Confirmed";
            }

            File.WriteAllLines(RESULTLOCATION + "OldExport.csv", lines);
        }
